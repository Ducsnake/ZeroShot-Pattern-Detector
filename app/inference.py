...
class PatternDetector:
    def __init__(self, config_path: str | Path | None = None, overrides: Mapping[str, Any] | None = None) -> None:
        ...
        self.template_matcher = TemplateMatcher(self.config.template)
    ...
    def predict(
        ...
        *,
        ...
        run_template = self.config.inference.enable_template if enable_template is None else enable_template
        ...
        if run_template and self.config.template.enabled:
            t0 = time.perf_counter()
            detections_resized.extend(self.template_matcher.match(pattern_pre, drawing_pre))
            timings["template_seconds"] = time.perf_counter() - t0
        else:
            timings["template_seconds"] = 0.0

        elapsed = time.perf_counter() - started
        skip_deep = (
            self.config.inference.skip_deep_if_candidates
            and any(det.score >= self.config.inference.deep_skip_score_threshold for det in detections_resized)
        )
        if run_deep and self.config.deep.enabled and not skip_deep and elapsed < self.config.inference.timeout_seconds:
            t0 = time.perf_counter()
